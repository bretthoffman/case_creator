import os, sys, zipfile, re, shutil, csv, json, uuid, time
...
from template_utils import (
    select_template, generate_id_block, map_material_to_xml, is_signature_doctor, inject_shade_into_materials
)
...
def should_zip(case_data, template_path, target_root) -> bool:
    ...
    # Either detect by template filename or by destination root:
    tpl_is_argen = "argen" in os.path.basename(template_path).lower()
    is_argen_dest = (target_root == SEND_TO_ARGEN_PATH)
...
def process_case(case_number, folder_path, log_callback=print):
	...
	# --- NEW: multi-unit safety check (fail fast) ---
        def _all_teeth_in_services(services):
            teeth = set()
            for s in services or []:
                for t in (s.get("toothlist") or []):
        ...
        teeth = _all_teeth_in_services(services)

        # also catch cases where EVO reports units > 1 even if toothlist is weird
        units_gt1 = any((s.get("units") or 0) > 1 for s in services)

        if len(teeth) > 1 or units_gt1:
            msg = "❌ Multiple units — manual import required"
        ...
        # --- NEW: material allowlist check (fail fast) ---
        # Only proceed if EVO indicates Argen Envision or Argen Adzir routes,
        # or an Argen Modeless family case.
        route = (case_data.get("material_hint", {}).get("route") or "").lower()
        allowed_routes = ("argen_envision", "argen_adzir", "modeless")

        if route not in allowed_routes:
            # Try to provide a useful hint for the GUI log (gold/alloy/emax, etc.)
            material_hint = None
            for s in (clean.get("services") or []):
                desc = (s.get("description") or "")
                dl = desc.lower()
                for kw in ("emax", "lithium disilicate", "gold", "alloy", "pfm", "full cast"):
                    if kw in dl:
                        material_hint = kw
                        break
                if material_hint:
            ...
            if material_hint:
                log_callback(f"❌ Manual import required — material: {material_hint}")
            else:
                log_callback("❌ Manual import required — unsupported material (not Envision/Adzir)")
            return "❌ Manual import required — material"
        # --- end NEW ---
        ...
        if not '3shape' in scanner:
            try:
                ...
                if is_case_in_jotform(first, last):
                    log_callback("🟡 JOTFORM CASE, requires manual import")
                    return "🟡 JOTFORM CASE, requires manual import"
                else:
        ...
        template_path = select_template(case_data)
        if "anterior" in template_path:
            log_callback("ANTERIOR")
        log_callback(f"📄 Using template: {os.path.basename(template_path)}")
        suffix = ""
        template_filename = os.path.basename(template_path).lower()
        scanner = case_data.get("scanner", "").lower()
        if "argen" in template_filename:
            suffix += "os"
        if "itero" in scanner:
            suffix += "i"
        ...
        elif "study" in template_filename:
            target_root = SEND_TO_1_9_PATH
        elif "anterior" in template_filename:
            target_root = SEND_TO_1_9_PATH
        elif "ai" in template_filename:
            target_root = SEND_TO_1_9_PATH
            debug("[route] AI template re-routed to DESIGNER path")
        else:
            raise ValueError(f"Unrecognized template route: {template_filename}")
        ...
        is_serbia_case = "brier creek" in doctor_name.lower()

        if "argen" in template_filename:
            log_callback("🏭 ARGEN CASE")
        elif ("study" in template_filename) and not is_serbia_case:
            log_callback("🧑‍🎓 DESIGNER CASE")
        elif ("study" in template_filename) and is_serbia_case:
            log_callback("🧑‍🎓 SERBIA CASE")
        elif ("anterior" in template_filename) and not is_serbia_case:
            log_callback("🧑‍🎓 DESIGNER CASE")
        elif ("anterior" in template_filename) and is_serbia_case:
            log_callback("🧑‍🎓 SERBIA CASE")
        elif "ai" in template_filename and not is_serbia_case:
            log_callback("🤖 DESIGNER CASE")
        elif "ai" in template_filename and is_serbia_case:
            log_callback("🤖 SERBIA CASE")
