...
def _settings_path() -> str:
    return os.path.join(_base_settings_dir(), "local_settings.json")


def load_local_settings() -> Dict[str, Any]:
    """
    Load local_settings.json from project root.
...
def _cached_settings() -> Dict[str, Any]:
    ...
    if _SETTINGS_CACHE is None:
        _SETTINGS_CACHE = load_local_settings()
    return _SETTINGS_CACHE


def save_settings_updates(updates: Dict[str, Any]) -> str:
    """
    Merge updates into local_settings.json while preserving unrelated keys.
...
def get_setting(key: str, default: Any) -> Any:
    """
    Return local override for key only when it is a non-empty value.
